...
export default {
   ...
   *    watch需要传递侦听源，watchEffect自动收集数据源作为依赖
   *    watch能监听到属性变化前后的值，watchEffect只能监听到属性变化后的值
   *    watch默认是惰性的，属性值变化时执行，watchEffect默认执行一次
   */
  /**
   * shallowRef，shallowReactive
   *    数据量非常大的时候，可以借助其实现非递归监听
   */
  /**
   * reactive
   *   reactive定义的响应式数据强制开启了深度监听，无法正确获取oldValue
   *   reactive定义的响应式数据中某个属性不会开启深度监听，需手动开启
   */
  setup() {
